...
def get_filename(excel_file_row, excel_filename = 'C:/Data/All Scan Notes New.xlsx', remote=False, temp=False):    
    ...
    # excel_filename = '//2.coltonlab.byu.edu/C$/Data/All Scan Notes New.xlsx'
        # If we already loaded it once, just return it
    global _all_scan_notes
    if _all_scan_notes is None:
        _all_scan_notes = pd.read_excel(excel_filename, parse_dates=[0])

    # print(str(_all_scan_notes['Date'][excel_file_row]))
    date = str(_all_scan_notes['Date'][excel_file_row])[:10] # <--------------------------------------------------------------------------
    sample = str(_all_scan_notes['Sample'][excel_file_row])
    # scan_type = str(all_scan_notes['Type of Scan'][excel_file_row])
    temperature = str(_all_scan_notes['Temp (K)'][excel_file_row])
    file = str(_all_scan_notes['File'][excel_file_row])
