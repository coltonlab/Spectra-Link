...
import processors.public.read_colton_files as rcf
...
def plot_EA_series(data, ax, colorbar=True, smooth=False, color_map_name='autumn_r'
    ...
    for i, voltage in enumerate(voltages):
        ...
        # ax.plot(X[50:],EA_data[50:], linewidth=linewidth, color=colors[i])#, label=f'{cmf.electric_field_kV_per_cm(voltage):.0f} kV/cm')


    # ax.set_xlim([1240/600, 1240/400])
...
def plot_ER_series(data, ax, colorbar=True, smooth=False, color_map_name='autumn_r'
                   , linewidth = 1, energy=True, phased=True,flip = False,text_size=14, export=False):
    # Getting the different voltage values
    ...
    for i, voltage in enumerate(voltages):
        ...
        # ax.plot(X[50:],EA_data[50:]*1000, linewidth=linewidth, color=colors[i])
...
def plot_gCD_temp_series(data, ax, colorbar=True, color_map_name='autumn_r', linewidth = 1, energy=True,smooth=False,text_size=14, phased=True, flip=False):
    # Get wavelength
    X = data['blank']['Digikrom Spectr.:0 (?)']
    ax.set_xlabel('Wavelength (nm)')    
    ...
    for temp in temperatures:
        ...
        ax.plot(X,EA_data/ABS_data1, linewidth=linewidth, color=colors[i])
        i += 1
    
    ax.tick_params(axis='both',which='major', labelsize=12)
    ax.axhline(y=0,color='k',linewidth=0.8)
    ax.set_ylabel('gCD',fontsize=text_size)
...
def plot_transmittance(data, ax, smooth = False, color='blue', energy=True,legend=None,value=1,text_size=14):
    ...
    # Calculate the absorption signal
    if smooth:
        ABS_data = cmf.transmition_smooth(data['trans']['R (V)'], data['blank']['R (V)'])    
    else:
        ABS_data = cmf.transmition(data['trans']['R (V)'], data['blank']['R (V)'])
...
def plot_data(data, ax, smooth = False, energy=True, legend=1):
    # Get wavelength
    ...
    if smooth:
        ...
        for data_key in data_keys:
            ax.plot(X, legend*data[data_key] ,label=f'{data_key}')
        # ax.plot(X,data['X (V)'],label = 'X (V)')
        # ax.plot(X,data['R (V)'],label = 'R (V)')
...
def plot_CD(data, ax, energy=True, Phased=True):
    ...
    # Calculate the EA signal
    CD_data = cmf.circular_dichrosim(data['X (V) Phased'] if Phased else data['X830 (V)'], data['Fluke (V)'])
