...
def phase_data(dataAll, x_name='X (V)', y_name='Y (V)'):
    ...
    logger.debug('Phased Angle: {:>7.2f} degrees'.format(min_angle * 180 / np.pi))
...
def phase_data_experiemnt(data, x_name='X (V)', y_name='Y (V)'):    
    ...
    # # This double four loop allows us to go through 60 values of Y to get an accuracy within 1.8e-4 degrees.
    for i in range(4): # Increase this number to increase your tolerance. Change this number if you want 1.8e-(n-2) order of accuracy
        angles = np.linspace(min_angle - BC, min_angle + BC, 10)

        # Looking for the smallest abs value of Y with in 10 data points 
        for angle in angles:
            # Vectorized calculation
            std_y = np.std(X * np.sin(angle) + Y * np.cos(angle))
    ...
    logger.debug(f'Phased Angle: {min_angle*180/np.pi}')
...
#     trans_smooth = savitzky_golay_smoothing(trans)
#     voltage_X_smooth = savitzky_golay_smoothing(voltage_X)
...
#     trans_smooth = savitzky_golay_smoothing(trans)
#     total_smooth = savitzky_golay_smoothing(total)
#     return trans_smooth / total
#
#
...
#     trans_smooth = savitzky_golay_smoothing(trans)
#     blank_smooth = savitzky_golay_smoothing(blank)
...
#     #     print(popt, pcov)
#     # else: # This else block is commented out in the original
...
#     logger.debug(f"TCSPC_decay_rate_2 popt: {popt2}")
#
...
#
#     logger.debug(f"TCSPC_decay_rate Chi-squared: {chi_squared:.3f}")
#
#     # print(decay_rate+decay_rate2)
#
#     import matplotlib.pyplot as plt
