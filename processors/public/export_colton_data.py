...
def Export_EA_series_data(data, filename, smooth_abs=False, smooth_ea=False, remote=False, flip=False, phased=False): 
    ...
    for i, voltage in enumerate(voltages):
        ...
        # Use the electric field corresponding to this voltage (electric_field may be an array)
        ef = electric_field[i]

        # Calculate the EA signal
        if smooth_ea:
            EA_calculations[f'EA {ef:.0f} kV/cm (mOD)'] = cmf.EA_smooth(data['voltages'][voltage]['X (V)'], data['trans']['R (V)'])
        else:
            EA_calculations[f'EA {ef:.0f} kV/cm (mOD)'] = cmf.EA(data['voltages'][voltage]['X (V)'], data['trans']['R (V)'])
...
def Export_ER_series_data(data, filename, smooth_abs=False, smooth_ea=False, remote=False, flip=False, phased=False): 
    """
    Exports EA data to a CSV file.
    Parameters:
    - data: Dictionary containing the data to be exported.
    ...
    for i, voltage in enumerate(voltages):
        ...
        # Calculate the EA signal
        if smooth_ea:
            EA_calculations[f'ER {ef:.0f} kV/cm'] = cmf.reflection_smooth(data['voltages'][voltage]['X (V)'], data['trans']['R (V)'])
        else:
            EA_calculations[f'ER {ef:.0f} kV/cm'] = cmf.reflection(data['voltages'][voltage]['X (V)'], data['trans']['R (V)'])
...
# create a dictionary to hold the EA calculations
    ABS_calculations = {}
    ABS_calculations['Energy (eV)'] = cmf.wavelength_to_energy(data['blank']['Digikrom Spectr.:0 (?)'])
...
def Export_EA_temp_data(data, filename, smooth=False, remote=False, flip=False, phased=False):
    # create a dictionary to hold the EA calculations
    EA_calculations = {}
    EA_calculations['Energy (eV)'] = cmf.wavelength_to_energy(data['blank']['Digikrom Spectr.:0 (?)'])
...
def Export_CD_temp_data(data, filename, smooth=False, remote=False, flip=False, phased=False):
    # create a dictionary to hold the EA calculations
    CD_calculations = {}
    CD_calculations['Energy (eV)'] = cmf.wavelength_to_energy(data['blank']['Digikrom Spectr.:0 (?)'])
